...
class LantentODE(Model):
    ...
    def __init__(
        ...
        self.func = _LatentODEfunc(latent_state_dim, latent_dynamics_dim, h_dim_ode).to(device)
        self.l_in_rec = _LatentInitRecognition(latent_state_dim, h_dim_x0, featurization_dim + 1).to(device)
        self.l_d_rec = _DynamicsRecognition(latent_dynamics_dim, h_dim_dynmcs, featurization_dim + 1).to(device)
        self.dec = _HeteroscedasticDecoder(latent_state_dim, h_dim_dec, state_dim).to(device)
    ...
    def _train(self, 
        ...
        params = (list(self.func.parameters()) + list(self.dec.parameters()) + list(self.l_in_rec.parameters()) + list(self.l_d_rec.parameters()))
        ...
        keys, traj_inputs, traj_outputs = _formulate_data_as_time_series(train_X, train_Y)
        ...
        if use_pretrained_model:
            ...
            for itr in pbar:
                optimizer.zero_grad()
                total_logpx, total_kl_z0, total_kl_zd = 0.0, 0.0, 0.0
                count = 0
                for (solvent_key, temp), traj_in, traj_o in tqdm(zip(keys, traj_inputs, traj_outputs)):
                    measurement_time = torch.atleast_1d(torch.Tensor(traj_in.to_numpy().T).squeeze()).to(self.device)
                    # exclude (near) duplicate time to allow odeint solve 
                    measurement_time, inverse_indices = _unique_with_tolerance(measurement_time) 
                    # get the embedding(featurization) of solvent
                    solvent_emb = train_X.loc[train_X['SOLVENT NAME'] == solvent_key, 'SOLVENT EMBEDDING'].iloc[0]
                    ...
                    analytic_kl_qz0 = _normal_kl(qz0_mean, qz0_std ** 2, pz0_mean, torch.ones_like(qz0_std)).sum(-1)
                    analytic_kl_qzd = _normal_kl(qzd_mean, qzd_std ** 2, pzd_mean, torch.ones_like(qzd_std)).sum(-1)
                    loss = torch.mean(-logpx + kl_weight * (analytic_kl_qz0 + analytic_kl_qzd), dim=0)
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(params, max_norm=kwargs.get('grad_clip_max_norm', 2.0))
                    optimizer.step()
...
class _LatentODEfunc(nn.Module):
    ...
    def __init__(self, latent_dim=4, latent_dnamics_dim=4, nhidden=20):
        self.latent_dnamics_dim = latent_dnamics_dim
...
def _formulate_time_series_as_data(
    ...
    for (solvent_name, temp), x_df, mean_tensor, var_tensor in zip(keys, x_series, pred_means, pred_vars):
        n_time = x_df.shape[0]
        ...
        traj_mean_df = pd.DataFrame(mean_np, columns=[f"state_{i}" for i in range(mean_np.shape[-1])])
        traj_var_df = pd.DataFrame(var_np, columns=[f"state_{i}" for i in range(var_np.shape[-1])])
    ...
    mean_df = mean_df.sort_values(["SOLVENT NAME", "Temperature", "Residence Time"]).reset_index(drop=True)
    var_df = var_df.sort_values(["SOLVENT NAME", "Temperature", "Residence Time"]).reset_index(drop=True)
