...
def plot_gp_covariance_matrix(model: GPModel, train_X: pd.DataFrame):
    ...
    train_X_solvents = pd.DataFrame(train_X["SOLVENT NAME"].unique(), columns=["SOLVENT NAME"])
    train_X_solvents[["Residence Time", "Temperature"]] = [0.0, 0.0]

    train_X_featurized = featurize_input_df(train_X_solvents, model.featurization, remove_constant=True)
